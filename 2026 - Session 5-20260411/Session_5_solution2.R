...
posterior <- extract(fit2, pars="x_sim")[[1]]
str(posterior)

muestra <- t(posterior) %>% as.data.frame() %>% 
  mutate(Any = 1860:1959) %>% 
  select(2001, sample(2:2000, 10)) %>% 
  gather(key = "Sim", value = "Valor", -Any)

ggplot(muestra) +
  geom_line(aes(Any, Valor, group = Sim), alpha = 0.3) +
  geom_line(data = df, aes(time, discoveries), color = "red")
...
posterior <- extract(fit3, pars="x_sim")[[1]]
str(posterior)
